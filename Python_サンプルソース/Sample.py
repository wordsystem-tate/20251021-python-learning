# 当ソースは Step1から Step6までを組み込んだ完成ソースコードとなります。
...
class VideoWindow(QWidget):
    ...
    def update_message(self):
        ...
        self.message_index = (self.message_index + 1) % len(self.messages)
            #<追加>
    def set_person_inf(self, query_result):    #<追加>
        # データの存在確認    #<追加>
        if query_result:    #<追加>
            row = query_result[0]    #<追加>
            name = str(row[1])    #<追加>
            age = str(row[2])    #<追加>
            cls = str(row[3])    #<追加>
            scores = str(row[4])    #<追加>
            text = f"名前：{name}  {age}歳 クラス：{cls} 成績：{scores}"    #<追加>
            self.pers_inf.setText(text)    #<追加>
        else:    #<追加>
            self.pers_inf.setText("個人情報が見つかりませんでした")    #<追加>
