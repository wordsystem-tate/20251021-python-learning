...
#顔データを保存
def save_face_data(file,encoding,names):
    with open(file,"wb")as f:
        pickle.dump((encoding,names),f)
...
class VideoWindow(QWidget):
    def __init__(self):
        ...
        if face_data:
            ...
            #tqdmを使ってフォルダ内の画像ファイルの処理状況を表示
            for foldername in tqdm(folder_list,desc=("Processing Folder")):
    ...
    def timerEvent(self,event):
        ...
        if event.timerId()==self.video_timer:
            ...
            if ret:
                ...
                for(top,right,bottom,left),face_encoding in zip(face_locations,face_encodings):
                    ...
                    if True in matches:
                        ...
                        if name not in opened_pages:
                            ...
                            engine.say("SELECT*FROM 個人情報 WHERE 名前 = %s",(name,))
                ...
                frame = cv2.convertScaleAbs(frame, alpha=0.95, beta=-10)#明るさを調節
    ...
    def set_person_inf(self,query_result):
        ...
        if query_result:
            ...
            self.pers_inf.setText("個人情報が見つかりません")
